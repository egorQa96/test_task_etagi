...
if(isset($data['do_signup'])) {
	...
	if(empty($errors)) {
		...
		$user->name = $data['name'];//Имя
		$user->family = $data['family'];//Фамилия
		$user->surname = $data['surname'];//Отчество
		if(!empty($_POST)){
		if(!empty($_POST['userrank'])){
			$user->userank = 1;
		}else{
			$user->userank = 0;
		}
...
<div class="container mt-4">
	   ...
	   <!-- Форма регистрации -->
		...
		<form action="signup.php" method="post">
			...
			<input type="text" class="form-control" name="surname" id="surname" placeholder="Введите отчество" required><br>
			<input type="password" class="form-control" name="password" id="password" placeholder="Введите пароль"><br>
			<input type="password" class="form-control" name="password_2" id="password_2" placeholder="Повторите пароль"><br>
			<label> Является ли пользователь Администратором </label>
			<input type="checkbox" class="form-control" name="userrank" id="userrank"><br>
